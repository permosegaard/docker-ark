...
EOF
  ...
  ulimit -n 2048 && cd /server/ && ShooterGame/Binaries/Linux/ShooterGameServer $( echo -n "TheIsland"; while read line; do echo -n "?${line}"; done < /server/server.cfg ) -server -log -UseBattlEye -usecache
fi
